...
The gateway uses a priority system to determine which provider to try first:

### Priority Levels

1. **BYOK (Bring Your Own Keys)** - Always tried first when you've configured provider keys
2. **PTB (Pay Through Billing)** - Helicone's managed keys using your credits

### Within Each Priority Level

Providers are ranked by:
1. **Cost** - Cheapest provider first
2. **Reliability** - Providers with better uptime
3. **Load balancing** - Equal-cost providers are rotated
...
1. Your OpenAI API key (if configured)
2. Your Azure OpenAI key (if configured)
3. Your AWS Bedrock credentials (if configured)
4. Helicone's OpenAI key (via credits)
5. Helicone's Azure key (via credits)
6. Helicone's AWS Bedrock credentials (via credits)
7. All other available providers
...
<Card title="Automatic Fallbacks" icon="shield-halved" href="/gateway/fallbacks">
  Learn about fallback strategies and error handling
</Card>
